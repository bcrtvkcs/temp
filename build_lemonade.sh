...
(cd "$HOME/crDroid" && . build/envsetup.sh && brunch lemonade && zcat "$HOME/crDroid/out/verbose.log.gz" > "$HOME/buildoutput_lemonade.log" && grep -Ei "fatal|error|unexpected|fail" "$HOME/buildoutput_lemonade.log" > "$HOME/builderrors_lemonade.txt")
